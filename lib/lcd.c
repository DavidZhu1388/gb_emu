...
void lcd_init() {
    ...
    ctx.ly_compare = 0;
    ctx.dma = 0;
    ctx.bg_palette = 0xFC; // default palette with all 4 colors
    ctx.obj_palette[0] = 0xFF; // default palette with all 4 colors
    ctx.obj_palette[1] = 0xFF; // default palette with all 4 colors
    ctx.win_x = 0;
    ctx.win_y = 0;
...
void lcd_write(u16 address, u8 value) {
    ...
    if (offset == 6) {
        // DMA transfer triggered at 0xFF46
        dma_start(value);
    }

    if (address == 0xFF47) {
        // update bg palette colors
        update_palette(value, 0);
    } else if (address == 0xFF48) {
        // update obj palette 1 colors
